...
public class JitsiMeetConferenceImpl
	...
	/**
	 * XMPP protocol provider used for the JVB connection. Unless configured with
	 * properties defined in {@link BridgeMucDetector} it wil be the same as
	 * {@link #protocolProviderHandler}
	 */
	 ...
	 *
	 * WARNING: To avoid deadlocks we must make sure that any code paths that lock
	 * both {@link #bridges} and {@link #participantLock} does so in the correct
	 * order. The lock on {@link #participantLock} must be acquired first.
	 */
	...
	/**
	 * Will re-allocate channels on the bridge for participant who signals ICE state
	 * 'failed'. New transport is sent in the 'transport-info' message similar to
	 * the conference migration scenario.
	 *
	 * {@inheritDoc}
	 */
	@Override
...
//if (//!fromJid.equals(toBeMutedJid) &&
	 ...
	 * allocate channels, and that the participants on a specific bridge need to be
	 * re-invited.
	 * 
	...
	class BridgeSession {
		 ...
		 *
		 * At the time of this writing it's used to distinguish between current and
		 * outdated ICE failed notifications coming from the client.
		 *
		 * It can often happen that during a bridge failure multiple clients will send
		 * ICE failed messages because all of them will have connectivity broken. Jicofo
		 * will mark the bridge as unhealthy when processing the first notification and
		 * any following ones should be discarded.
		 */
		 ...
		 * {@link #terminate(AbstractParticipant, boolean)}) for all participants.
		 * 
		 ...
		 *                    channels.
		 * @param syncExpire  - whether or not the Colibri channels should be expired in
		 *                    a synchronous manner, that is with blocking the current

...
public class ShibbolethAuthAuthority
     ...
     * AuthBundleActivator#LOGOUT_URL_PNAME} in order to use default Shibboleth
     * URLs for login and logout. It can not be skipped, because Shibboleth will
     ...
     * %1$s - *machineUID* that is identifier of users machine which can be used
     * by the system to distinguish between session for the same login on
     ...
     * %4$s - *popup* indicates if this URL will be opened in a popup. In this
     * case session-id can be immediately passed to parent window using CORS
     * window messages.<br/>
     * Example:<br/>
     ...
     * distinguish between the sessions for the same login name on different
     * machines.
